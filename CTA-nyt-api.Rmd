...
We are now going to work with a private API, using  we will take the New York Times Article Search API as an example. This API allows users to search articles by string and dates, and returns counts of articles and a short description of each article (but not the full text). You can create a new account and obtain a key [here](https://developer.nytimes.com/get-started). Create an `.env` file in your **local** directory, and save your key. Remember, be sure never to display any file publicly, e.g. in a public repo or a private collaborative repo, when it contains an active key!

```{r}
# read_______(".env")
# apikey <- Sys.____("API_KEY")
# 
# # Read the .env file
# dotenv::load_dotenv(".env")  # This loads environment variables from .env

apikey <- Sys.getenv("API_KEY")  # Retrieve API key
print(apikey)  # Check if it's loaded correctly (avoid printing secrets in real use)
```

The first step is to identify the base URL of the endpoint and the parameters that we can use to query the API, for the Article Search API you can find this URL structure [here](https://developer.nytimes.com/docs/articlesearch-product/1/overview). Now we can do a first API call using the **httr** package.
...
json <- content(r, "parsed")  # Extracts the JSON response as a structured list
class(json)  
length(json)  # List with 3 elements

json$status  # This should be "OK"

# str(json$response)  # Shows the structure of the response object
# invisible(str(json$response))
names(json$response)

length(json$response$docs)  # The number of returned documents (default: max 10)

json$response$meta  # Metadata of the request (e.g., "hits" = total articles available)
```
...
nyt_years_count <- function(q, yearinit, yearend) {
  ...
  for (y in years) {
   ...
   # Return a dataframe with year and article count
...
```{r}
counts_inequality <- nyt_years_count(q = "inequality", yearinit = 2015, yearend = 2020)
```

```{r}
# ggplot() +
#   geom_line(aes(x = 2015:2020, y = counts_inequality)) +
#   labs(title = "Mentions of inequality in the NYT by year",
#        y = "Article count",
#        x = "") +
#   theme_minimal()

ggplot(counts_inequality, aes(x = year, y = count)) +
  geom_line(color = "blue", size = 1) +
  geom_point(color = "red", size = 2) +
  labs(title = "Mentions of 'Inequality' in NYT (2015-2020)",
       y = "Article Count",
       x = "Year") +
  theme_minimal()
```
...
nyt_dates_count <- function(q, init, end, by){
  ...
  dates <- seq(from = as.Date(init), to = as.Date(end), by = by)
...
```{r}
counts <- nyt_dates_count(q = "obama", init = "2007-01-01", 
                          end = "2012-12-31", by = "month")
```
...
ggplot(counts, aes(x = date, y = count)) +
  ...
  labs(title = "Mentions of 'Obama' in the NYT by month",
       x = "Month",
       y = "Article count") +
  geom_vline(xintercept = as.numeric(as.Date("2007/02/10")), linetype = "dashed") +
  geom_vline(xintercept = as.numeric(as.Date("2008/08/27")), linetype = "dashed", color = "red") +
  geom_vline(xintercept = as.numeric(as.Date("2008/11/04")), linetype = "dashed", color = "darkred") +
  theme_minimal()
```
